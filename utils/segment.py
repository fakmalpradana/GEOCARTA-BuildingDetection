...
class Predict:
    ...
    def combine_json(overlap:float, tipe= 'masks' or 'boxes'):
        ...
        for i in range(len(lst)):
            ...
            ann = jsonYOLO(f'public/patch-temp/patch{i+1001}.json').load()
            ...
            for k in range(len(ann)):
                    isian = {
                        'id': f'{i}_{k+100}',
                        'img_path': f'public/patch-temp/patch{k+1001}.png',
                        'hasil': ann[k].tolist(),
                    }
                    list_hasil.append(isian)

            with open(f'public/patch-temp/patch{i+1001}.json', 'w') as output_file:
                json.dump(list_hasil, output_file, indent=2, cls=NumpyArrayEncoder)
            

        for i in range(len(lst)):
            if i == 0:
                with open(f'public/patch-temp/patch{i+1001}.json', 'r') as f:
                    arr = json.load(f)
